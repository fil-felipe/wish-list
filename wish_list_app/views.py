...
def add_child_user(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid:
            ...
            return redirect(
                "wish_list_app:child_register_done", username=new_child_user.username
            )
    else:
        form = form_object()
...
def child_register_done(request, username):
    return render(
        request, "child_register/child_register_done.html", {"username": username}
    )
...
def add_wish_list(request, username=None):
    ...
    if request.method == "POST":
        ...
        if form.is_valid:
            ...
            new_wish_list.list_name = (
                f"{new_wish_list.list_user.first_name} {new_wish_list.list_user.last_name} "
    ...
    return render(
        request,
        "wish_list_app/wish_lists/add_wish_list.html",
        {"user": list_user_object, "form": form},
    )
...
def view_all_wish_lists(request):
    ...
    return render(
        request,
        "wish_list_app/wish_lists/all_wish_lists.html",
...
def view_wish_list(request, username):
    ...
    return render(
        request,
        "wish_list_app/wish_lists/wish_lists.html",
        {"user": list_user_object, "user_wish_lists": user_wish_lists},
    )


@login_required
def view_wish_list_gifts(request, username, list_name):
    gift_list_object = get_object_or_404(
        WishList, list_user__username=username, slug=list_name
    )
    gift_list = gift_list_object.gift_item.filter(reserved=False)
    return render(
        request,
        "wish_list_app/gifts/gift_list.html",
        {"gift_list": gift_list, "list_name": gift_list_object},
    )
...
def add_gift(request, list_name=None):
    ...
    return render(
        request,
        "wish_list_app/gifts/add_gift.html",
        {"list": list_object, "form": form},
    )
...
def reserve_gift(request, gift_id):
    ...
    if request.method == "POST":
        gift.reserved = True
        gift.reserved_time = datetime.now()
        gift.reserved_user = request.user
        gift.save()
        return redirect(gift.get_absolute_url())

    return render(request, "wish_list_app/gifts/reserve_gift.html", {"gift": gift})
...
def edit_gift(request, username, list_name, gift_slug):
    ...
    return render(
        request, "wish_list_app/gifts/edit_gift.html", {"form": form, "gift": gift}
    )
...
def delete_gift(request, gift_id):
    ...
    if request.method == "POST":
        if (
            request.user == gift.created_by
            or request.user == gift.gift_list.list_creator
        ):
