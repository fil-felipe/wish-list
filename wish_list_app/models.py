...
class WishList(models.Model):
    ...
    list_user = models.ForeignKey(
        User,
        on_delete=models.CASCADE,
        related_name="gift_lists_user",
        verbose_name="Obdarowany",
    )
    list_creator = models.ForeignKey(
        User, on_delete=models.CASCADE, blank=True, related_name="gift_lists_creator"
    )
    ...
    def get_absolute_url(self):
        return reverse(
            "wish_list_app:gift_lists", args=[self.list_user.username, self.slug]
        )
...
class Gift(models.Model):
    gift_list = models.ForeignKey(
        WishList,
        on_delete=models.CASCADE,
        related_name="gift_item",
        verbose_name="Lista prezentów",
    )
    ...
    reserved_user = models.ForeignKey(
        User, models.SET_NULL, blank=True, null=True, verbose_name="Osoba rezerwująca"
    )
    ...
    def get_absolute_url(self):
        return reverse(
            "wish_list_app:gift_lists",
            args=[self.gift_list.list_user.username, self.gift_list.slug],
        )

    def get_reserved_url(self):
        return reverse(
            "wish_list_app:reserve_gift",
            args=[self.gift_list.list_user.username, self.gift_list.slug, self.slug],
        )

    def get_edit_url(self):
        return reverse(
            "wish_list_app:edit_gift",
            args=[self.gift_list.list_user.username, self.gift_list.slug, self.slug],
        )
